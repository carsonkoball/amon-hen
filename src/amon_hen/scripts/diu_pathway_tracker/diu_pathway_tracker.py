...
def _get_pathway_ids():
    ...
    cso_pathway_ids = [cso_pathway["href"].split('/')[-1] for cso_pathway in cso_pathways]
    
    # Find CCAO Pathways
    ccao_pathways = soup.find_all(class_="pathway")[4].find_all('a')
    ccao_pathway_ids = [ccao_pathway["href"].split('/')[-1] for ccao_pathway in ccao_pathways]

    logger.debug("Retrieved %d CSO pathways and %d CCAO pathways", len(cso_pathway_ids), len(ccao_pathway_ids))
...
def _diu_pathway_tracker(tracker):
    ...
    for index, pathway_id in enumerate(cso_pathway_ids + ccao_pathway_ids):
        ...
        current_cso_active.add(pathway_id) if is_cso_pathway else current_ccao_active.add(pathway_id)
        
        pathway_path = config.CSO_PATHWAY_DIR(pathway_id) if is_cso_pathway else config.CCAO_PATHWAY_DIR(pathway_id)
        ...
        if result.has_changed:
            ...
            if result.is_new:
                ...
                entry = config.HISTORY_ENTRY(
                    ...
                    pathway_info={
                        "title": result.new_data["title"]
                    },
                    event="added",
                )

                _append_to(entry=entry, path=config.CSO_HISTORY_FILE if is_cso_pathway else config.CCAO_HISTORY_FILE)
    
    # Load previously active pathways
    previous_cso_active, previous_ccao_active = _load_active()

    # Save current active pathways
    _save_active(current_cso_active=current_cso_active, current_ccao_active=current_ccao_active)
